...
func (ctx *TagService) GetTag(id uint) (model.Tag, error) {
	...
	for i, _ := range tag.Articles {
		tag.Articles[i].Content = ""
	}
...
func (ctx *TagService) GetTags(pageNo int, pageSize int) (model.Page, error) {
	...
	var totalSize int
	if err := db.Table("tags").Where("deleted_at is null").Count(&totalSize).Error; err != nil {
	...
	for i, _ := range tags {
		for j, _ := range tags[i].Articles {
			tags[i].Articles[j].Content = ""
		}
	}
